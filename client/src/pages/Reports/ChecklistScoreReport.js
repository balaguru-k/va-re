...
const ChecklistScoreReport = () => {
    ...
    const fetchReport = async (page = 1, limitOverride) => {
        ...
        try {
            ...
            const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/reports/checklist-scores?${params.toString()}`, {
    ...
    const columnOptions = [
        ...
        { value: 'yes_score', label: 'Yes Score' },
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/reports/checklist-scores/export?${params.toString()}`, {
                headers: { Authorization: `Bearer ${token}` },
                responseType: 'blob'
            });
            
            // Overwriting buildParams call in handleExport to ensure it uses tableFilters

            const blob = new Blob([response.data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
            const url = window.URL.createObjectURL(blob);
            const link = document.createElement('a');
            link.href = url;
            link.download = `Checklist_Score_Report_${new Date().toISOString().split('T')[0]}.xlsx`;
    ...
    return (
        <div className="container mx-auto px-4 py-2">
            <div className="mb-8 flex justify-between items-center">
                <h1 className="text-2xl font-bold text-gray-800">Checklist Score Report</h1>
            ...
            <div className="bg-white border border-red-100 rounded-lg min-h-[450px]">
                <div className="px-4 py-3 border-b border-gray-200 bg-gray-50">
                    <div className="flex justify-between items-center mb-2">
                        <h2 className="text-sm font-medium text-gray-700">Checklist Score Results</h2>
                        ...
                        <table className="min-w-full">
                            <thead style={{ backgroundColor: '#efeeee' }} className="border-b border-gray-200 sticky top-0 text-left">
                                <tr>
                                    ...
                                    <th className="px-4 py-3 text-left text-sm font-medium text-gray-700 text-left">Yes Score</th>
                                    <th className="px-4 py-3 text-left text-sm font-medium text-gray-700 text-left">No Score</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-100 text-left">
                                {filteredData.map((item, index) => (
                                    <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                                        ...
                                        <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-green-600">{item.yes_score}</td>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-red-600">{item.no_score}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
...
export default ChecklistScoreReport;
