...
const CompliancePendingTickets = () => {
  ...
  const fetchTickets = () => {
    ...
    api.get('/compliance/dashboard')
      .then(res => {
        ...
        // Filter only pending tickets using effective status logic
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 space-y-3">
      ...
      <PageHeader title="Pending Tickets">
          ...
          <table style={{ minWidth: '1400px' }} className="w-full">
            <thead style={{ backgroundColor: '#ededed' }} className="border-b border-gray-200">
              <tr>
                ...
                <th className="px-4 py-4 text-left text-sm font-medium text-gray-700 sticky top-0 z-10 cursor-pointer select-none" style={{ backgroundColor: '#ededed' }} onClick={() => handleSort('raised_at')}>Raised On<SortIcon col="raised_at" /></th>
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                <tr><td colSpan="13" className="px-4 py-6 text-center text-gray-500">No pending tickets found</td></tr>
              ) : (
                paginated.map((ticket, index) => {
                  ...
                  return (
                    <tr key={ticket.id} className={`cursor-pointer hover:bg-gray-100 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`} onClick={() => handleView(ticket)}>
                      ...
                      <td className="px-4 py-3 text-sm text-gray-600 whitespace-nowrap">{ticket.raised_at ? formatDateTime(ticket.raised_at) : '-'}</td>
                ...
                <div>
                  ...
                  <div className="px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm">
                    {(() => {
                      ...
                      return displayStatus ? (
                        <span className="inline-flex px-2 py-0.5 text-xs font-medium rounded-full whitespace-nowrap bg-yellow-100 text-yellow-700">
                          {displayStatus}
                        </span>
                      ) : '-';
                    })()}
...
export default CompliancePendingTickets;