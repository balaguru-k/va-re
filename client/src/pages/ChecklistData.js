...
const ChecklistData = () => {
  ...
  const fetchChecklistsData = async () => {
    try {
      setLoading(true);
      const response = await checklistAPI.getAllChecklistsData(fromDate, toDate);
  ...
  if (window.innerWidth <= 768) {
    return (
      <div className="flex flex-col items-center justify-center h-full py-20 px-6 text-center">
        ...
        <p className="text-sm text-gray-500">Please use a desktop browser to access Checklists.</p>
      </div>
    );
  }
