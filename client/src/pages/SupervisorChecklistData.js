...
const SupervisorChecklistData = () => {
  ...
  const fetchChecklistsData = async () => {
    try {
      setLoading(true);
      const response = await checklistAPI.getSupervisorChecklistsData(fromDate, toDate);
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Supervisor Checklists">
                ...
                checklistsData.map((item, index) => (
                  <tr key={item.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    <td className="px-4 py-3">
                      <button
                        onClick={() => navigate(`/supervisor-checklist/${item.id}`)}
...
export default SupervisorChecklistData;